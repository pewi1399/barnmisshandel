...
UTAN_oppen = FALSE
if(UTAN_oppen){
  load(file = file.path(path,"Output","DMP","DMP_100_barndiagnoserUtanOppen.Rdata")) #dia5
}else{
...
timestamp <- gsub(" ", "_",gsub("-|:", "",as.character(Sys.time())))

if(UTAN_oppen){
  
  
  write_dta(out, file.path(path,"Output","DMP",paste0("Analysdata",timestamp,"UtanOppen.dta")))
  saveRDS(out, file.path(path,"Output","DMP","AnalysdataUtanOppen.rds"))
  ...
  save(out1, file = file.path(path,"Output","DMP","diagnosinnanUtanOppen.rdata"))
  #table(dat9$barnMisshandel_d1)
}else{
  write_dta(out, file.path(path,"Output","DMP",paste0("Analysdata", timestamp,".dta")))
  ...
  #table(dat9$barnMisshandel_d1)5 
}
