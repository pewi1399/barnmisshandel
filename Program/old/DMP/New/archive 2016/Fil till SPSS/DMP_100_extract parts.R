...
dia2 = dia1[,list(rakit_d1 = sum(rakit_d1,na.rm=T),
                  frakturRevben_d1 = sum(frakturRevben_d1,na.rm=T),
                  frakturLårben_d1 = sum(frakturLårben_d1,na.rm=T),
                  frakturUnderben_d1 = sum(frakturUnderben_d1,na.rm=T),
                  övrigSkallskada_d1 = sum(övrigSkallskada_d1,na.rm=T),
                  barnMisshandel_d1 = sum(barnMisshandel_d1,na.rm=T),
                  UtrObsMissh_d1 = sum(UtrObsMissh_d1, na.rm=T),
                  DiagnosMissh_d1 = sum(DiagnosMissh_d1, na.rm=T),
                  barnmisshandelssyndromUNS = sum(barnmisshandelssyndromUNS,na.rm=T),
                  syndromBarnMisshdel = sum(syndromBarnMisshdel,na.rm=T),
                  frakturÖverUnderArm_d1 = sum(frakturÖverUnderArm_d1,na.rm=T),
                  skallFraktur_d1 = sum(skallFraktur_d1,na.rm=T),
                  retinalBlödning_d1 = sum(retinalBlödning_d1,na.rm=T),
                  subduralIT_d1 = sum(subduralIT_d1,na.rm=T),
                  subduralTraum_d1 = sum(subduralTraum_d1,na.rm=T),
                  svt_d1 = sum(svt_d1,na.rm=T),
                  ogi_d1 = sum(ogi_d1,na.rm=T),
                  spädbarnskolik_d1 = sum(spädbarnskolik_d1,na.rm=T),
                  frakturRörben_d2 = sum(frakturRörben_d2,na.rm=T),
                  subduralBlödning_d2 = sum(subduralBlödning_d2,na.rm=T),
                  ALTE = sum(ALTE,na.rm=T),
                  kramper = sum(kramper,na.rm=T),
                  våldtäkt = sum(våldtäkt,na.rm=T),
                  feberkramp = sum(feberkramp,na.rm=T),
                  krampanfallUNS = sum(krampanfallUNS,na.rm=T),
                  epilepsi = sum(epilepsi,na.rm=T),
                  glaskroppsblödning = sum(glaskroppsblödning,na.rm=T),
                  subkonjunktivalBlödning = sum(subkonjunktivalBlödning,na.rm=T),
                  hjärnskakning = sum(hjärnskakning,na.rm=T),
                  frakturLändkotpelare = sum(frakturLändkotpelare,na.rm=T),
                  nyckelbenfraktur = sum(nyckelbenfraktur,na.rm=T),
                  kräkningar = sum(kräkningar,na.rm=T),
                  infektion = sum(infektion,na.rm=T),
                  neutropeni = sum(neutropeni,na.rm=T),
                  anemi = sum(anemi,na.rm=T),
                  blöjderytemDermatit = sum(blöjderytemDermatit,na.rm=T),                       
                  kraniosunostos = sum(kraniosunostos,na.rm=T),                       
                  missbildningar = sum(missbildningar,na.rm=T),
                  obstipation = sum(obstipation,na.rm=T),
                  gulsot = sum(gulsot,na.rm=T),
                  failureToThrive = sum(failureToThrive, na.rm=T),
                  försenatKroppsligtUtvecklingsstadium = sum(försenatKroppsligtUtvecklingsstadium,na.rm=T),                  
                  ickeSpecSymtomNervMuskoskelSys = sum(ickeSpecSymtomNervMuskoskelSys,na.rm=T),
                  blåttÖga = sum(blåttÖga, na.rm=T),
                  ytligSkadaHuvudBröst = sum(ytligSkadaHuvudBröst,na.rm=T),                       
                  kontusionRyggBäckenBukväggLårUnderben = sum(kontusionRyggBäckenBukväggLårUnderben,na.rm=T),
                  brännskada = sum(brännskada, na.rm=T),
                  anoxiskHjärnskada = sum(anoxiskHjärnskada,na.rm=T),
                  skadorPåHuvudetUtanFraktur = sum(skadorPåHuvudetUtanFraktur,na.rm=T),
                  andningsuppehåll = sum(andningsuppehåll,na.rm=T),
                  epilepsi = sum(epilepsi,na.rm=T),
                  ytligSårskada = sum(ytligSårskada,na.rm=T),
                  fallskadaKlämskada = sum(fallskadaKlämskada,na.rm=T),
                  kontaktHetDryckFöda = sum(kontaktHetDryckFöda,na.rm=T),
                  misshandelUtövadFörälder = sum(misshandelUtövadFörälder, na.rm=T),
                  misshandelUtövadIckeSpecPerson = sum(misshandelUtövadIckeSpecPerson, na.rm=T),
                  mordDråpförsök = sum(mordDråpförsök, na.rm=T),
                  olycka = sum(olycka, na.rm=T),
                  andraSkadorOklarAvsikt = sum(andraSkadorOklarAvsikt, na.rm=T),
                  psykiskStörningRubbningIFamilj = sum(psykiskStörningRubbningIFamilj, na.rm=T),
                  summationVariabler_d1 =  sum(summationVariabler_d1, na.rm=T),
                  maltreatmentSyndrome =  sum(maltreatmentSyndrome, na.rm=T),
                  assault =  sum(assault, na.rm=T),
                  undeterminedCauses =  sum(undeterminedCauses, na.rm=T),
                  adverseSocialCircumstances =  sum(adverseSocialCircumstances, na.rm=T),
                  dottBarn =  sum(dottBarn, na.rm=T),
                  maltreatmentRelatedInjury =  sum(maltreatmentRelatedInjury, na.rm=T),
                  markerintracranical =  sum(markerintracranical, na.rm=T),
                  markerlongbone =  sum(markerlongbone, na.rm=T),
                  frakturHumerusskaftet =  sum(frakturHumerusskaftet, na.rm=T),
                  frakturFemurskaftet =  sum(frakturFemurskaftet, na.rm=T),
                  s_aktivRakit = sum(s_aktivRakit ,na.rm=TRUE),
                  s_osteomalaci = sum(s_osteomalaci ,na.rm=TRUE),
                  s_kalcium_d1 = sum(s_kalcium_d1 ,na.rm=TRUE),
                  s_george_d1 = sum(s_george_d1 ,na.rm=TRUE),
                  s_hie = sum(s_hie ,na.rm=TRUE),
                  s_cpSkada = sum(s_cpSkada ,na.rm=TRUE),
                  s_anoxiskHjarnskad = sum(s_anoxiskHjarnskad ,na.rm=TRUE),
                  s_krampOspec = sum(s_krampOspec ,na.rm=TRUE),
                  s_krampNyfodd = sum(s_krampNyfodd ,na.rm=TRUE),
                  s_Epdiag = sum(s_Epdiag ,na.rm=TRUE),
                  s_enbartFeberKramp = sum(s_enbartFeberKramp ,na.rm=TRUE),
                  s_IVH = sum(s_IVH ,na.rm=TRUE),
                  s_kramperNeo = sum(s_kramperNeo ,na.rm=TRUE),
                  s_kardiovaskMissb = sum(s_kardiovaskMissb ,na.rm=TRUE),
                  s_hjartsvikt = sum(s_hjartsvikt ,na.rm=TRUE),
                  s_njursjukdom = sum(s_njursjukdom ,na.rm=TRUE),
                  s_kroniskNjursvikt = sum(s_kroniskNjursvikt ,na.rm=TRUE),
                  s_anemi = sum(s_anemi ,na.rm=TRUE),
                  s_anemiUnderburenJarnbrist = sum(s_anemiUnderburenJarnbrist ,na.rm=TRUE),
                  s_anemiOspec = sum(s_anemiOspec ,na.rm=TRUE),
                  s_sepsis = sum(s_sepsis ,na.rm=TRUE),
                  s_uppfodningSvarighet = sum(s_uppfodningSvarighet ,na.rm=TRUE),
                  s_bristMineralamnen = sum(s_bristMineralamnen ,na.rm=TRUE),
                  s_rubbningarKalcium = sum(s_rubbningarKalcium ,na.rm=TRUE),
                  s_annanNaringsbristSpec = sum(s_annanNaringsbristSpec ,na.rm=TRUE),
                  s_proteinEnergibrist = sum(s_proteinEnergibrist ,na.rm=TRUE),
                  s_undernutritionNyfodd = sum(s_undernutritionNyfodd ,na.rm=TRUE),
                  s_annanUteblivenNormalUtveckl = sum(s_annanUteblivenNormalUtveckl ,na.rm=TRUE),
                  s_failureToThrive = sum(s_failureToThrive ,na.rm=TRUE),
                  s_rubbningKalciumMagnesiumNyfodd = sum(s_rubbningKalciumMagnesiumNyfodd ,na.rm=TRUE),
                  s_neonatalHypokalcHypomagnesemi = sum(s_neonatalHypokalcHypomagnesemi ,na.rm=TRUE),
                  s_annanHypokalcemi = sum(s_annanHypokalcemi ,na.rm=TRUE),
                  s_gulsotForlForeBeraknad = sum(s_gulsotForlForeBeraknad  ,na.rm=TRUE),
                  s_gulsotNyfoddOspec = sum(s_gulsotNyfoddOspec ,na.rm=TRUE),
                  s_endokrinRubbningHypokalc = sum(s_endokrinRubbningHypokalc ,na.rm=TRUE),
                  s_bristDVitamin = sum(s_bristDVitamin ,na.rm=TRUE),
                  s_rakit_d1 = sum(s_rakit_d1 ,na.rm=TRUE),
                  s_rakit_d2 = sum(s_rakit_d2 ,na.rm=TRUE),
                  s_DIAG26500 = sum(s_DIAG26500 ,na.rm=TRUE)
),
by=c("lpnr")]
...
timedata = dia0[,c("lpnr",kalenderVars)]
